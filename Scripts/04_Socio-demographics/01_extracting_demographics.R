...
# Used files from Wave 4 and harmonised datafiles because data on education and paternal occupation
# were already divided as per our criteria in the harmonized datafile
...
Harmonised_datafile <- read.csv(here("Data", "Raw_Data", "Original_ELSA_Data", "tab", "h_elsa.tab"),
                                header = TRUE, sep = "\t")

Harmonised_datafile <- Harmonised_datafile %>%
  dplyr::select(raeducl, idauniq) # extracting pre-categorised data on education

Combined_Wave4 <- list(Wave_4_datafile, Wave_4_finance_datafile, Harmonised_datafile) %>%
  reduce(inner_join, by = c("idauniq"))

Wave_4_SES <- Combined_Wave4 %>%
  dplyr::select(idauniq, difjobd, w4nssec3, nettotw_bu_s, raeducl)
head(Wave_4_SES)
...
# on ISCED codes, see www.uis.unesco.org and the OECD document entitled "Classifying Educational
# Programmes: Manual for ISCED-97 Implementation in OECD Countries, 1999 Edition".
# RAEDUCL is coded as Demographics, Identifiers, and Weights  75
# follows: 1.Less than lower secondary education, 2.Upper secondary & vocational training, and 3.Tertiary
# education.

Wave_4_SES <- Wave_4_SES %>%
  mutate(Education_Upper_Second = ifelse(Wave_4_SES[, 5] <0, -99, ifelse(Wave_4_SES[, 5] == 2, 1, 0))) %>%
  mutate(Education_Tertiary = ifelse(Wave_4_SES[, 5] <0, -99, ifelse(Wave_4_SES[, 5] == 3, 1, 0)))
